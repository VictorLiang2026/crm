...
4. **部署前端**：`tcb hosting deploy ./admin.html /crm/admin.html -e crm-d1gkae8ddc930d151 --yes`（或控制台静态托管上传到 `crm/` 目录，并填入 `envId`）；**单文件无构建步骤**。访问地址：`https://crm-d1gkae8ddc930d151-1434199662.tcloudbaseapp.com/crm/admin.html`（多系统按目录隔离，根目录仅保留 `__auth/` 平台文件）
...
- **前端**：MCP `manageHosting` 上传 `admin.html` 到静态托管 `crm/` 目录（CLI 等价命令：`tcb hosting deploy ./admin.html /crm/admin.html`，显式指定单文件云路径，不会泄露 `.git/`、`cloudfunctions/` 等非托管文件）。
...
- 部署后核对线上文件 MD5（裸 URL 即可，admin.html 已设置 no-store/no-cache；⚠️ 网关对带 `?v=`/`?t=` 查询参数的 URL 返回 404）。
