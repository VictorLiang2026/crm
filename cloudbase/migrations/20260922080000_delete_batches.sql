-- Public-only batch deletion. No historical backfill; existing view columns remain unchanged.
BEGIN;
SET LOCAL lock_timeout='5s';
ALTER TABLE public.customers ADD COLUMN delete_batch_id uuid;
COMMENT ON COLUMN public.customers.delete_batch_id IS 'UUID of the current soft-delete operation; NULL for active or legacy rows';
ALTER TABLE public.followups ADD COLUMN delete_batch_id uuid;
COMMENT ON COLUMN public.followups.delete_batch_id IS 'UUID of the current soft-delete operation; NULL for active or legacy rows';
ALTER TABLE public.gifts ADD COLUMN delete_batch_id uuid;
COMMENT ON COLUMN public.gifts.delete_batch_id IS 'UUID of the current soft-delete operation; NULL for active or legacy rows';
ALTER TABLE public.photos ADD COLUMN delete_batch_id uuid;
COMMENT ON COLUMN public.photos.delete_batch_id IS 'UUID of the current soft-delete operation; NULL for active or legacy rows';
ALTER TABLE public.policy_review_reports ADD COLUMN delete_batch_id uuid;
COMMENT ON COLUMN public.policy_review_reports.delete_batch_id IS 'UUID of the current soft-delete operation; NULL for active or legacy rows';
ALTER TABLE public.ocr_records ADD COLUMN delete_batch_id uuid;
COMMENT ON COLUMN public.ocr_records.delete_batch_id IS 'UUID of the current soft-delete operation; NULL for active or legacy rows';
ALTER TABLE public.products ADD COLUMN delete_batch_id uuid;
COMMENT ON COLUMN public.products.delete_batch_id IS 'UUID of the current soft-delete operation; NULL for active or legacy rows';
ALTER TABLE public.recruit_candidates ADD COLUMN delete_batch_id uuid;
COMMENT ON COLUMN public.recruit_candidates.delete_batch_id IS 'UUID of the current soft-delete operation; NULL for active or legacy rows';
ALTER TABLE public.recruit_followups ADD COLUMN delete_batch_id uuid;
COMMENT ON COLUMN public.recruit_followups.delete_batch_id IS 'UUID of the current soft-delete operation; NULL for active or legacy rows';
